...
[RHQ-Metrics UI Quickstart Video](https://vimeo.com/101576357)   _Length - 8:33_
...
There are some unit tests in the project as well as integration tests in the `rest-test/` directory.
You can select the backend implementation (Cassandra, Memory) by passing the parameter `-Dbackend=mem` or
`-Dbackend=cass` to the Maven command line. If the parameter is omitted, the Memory backend is selected.
...
RHQ-metrics is http://twitter.com/#!/rhq_Project/[@rhq-Project] on Twitter.
