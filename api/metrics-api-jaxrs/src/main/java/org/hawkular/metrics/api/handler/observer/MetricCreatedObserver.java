...
public class MetricCreatedObserver extends EntityCreatedObserver<MetricAlreadyExistsException> {
    ...
    public MetricCreatedObserver(RoutingContext ctx, URI location) {
        super(
                ctx,
                location,
    ...
    private static String getMetricAlreadyExistsResponse(MetricAlreadyExistsException e) {
        return "A metric with name [" + e.getMetric().getMetricId().getName() + "] already exists";
    }
}
