...
public class AvailabilityHandler {
    ...
    public Response findRawData(QueryRequest query) {
        ...
        List<MetricId<AvailabilityType>> metricIds = query.getIds().stream().map(id -> new MetricId<>(tenantId,
                AVAILABILITY, id))
                .collect(toList());
        Observable<NamedDataPoint<AvailabilityType>> dataPoints = metricsService.findDataPoints(metricIds,
                timeRange.getStart(), timeRange.getEnd(), limit, order).observeOn(Schedulers.io());

        StreamingOutput stream = output -> {
            JsonGenerator generator = mapper.getFactory().createGenerator(output, JsonEncoding.UTF8);
            CountDownLatch latch = new CountDownLatch(1);
            dataPoints.subscribe(new NamedDataPointObserver<>(generator, latch, AVAILABILITY));
