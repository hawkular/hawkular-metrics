...
class NumericMetricStatisticsITest extends RESTTest {
  ...
  void shouldNotAcceptInvalidParams() {
    ...
    badGet(path: "${tenantId}/guage/$metric/data", query: [buckets: 0]) { exception ->
      // Bucket count = zero
      assertEquals(400, exception.response.status)
    }

    badGet(path: "${tenantId}/guage/$metric/data", query: [buckets: Integer.MAX_VALUE]) { exception ->
      // Bucket size = zero
      assertEquals(400, exception.response.status)
    }

    badGet(path: "${tenantId}/guage/$metric/data", query: [bucketDuration: "1w"]) { exception ->
      // Illegal duration
      assertEquals(400, exception.response.status)
    }

    badGet(path: "${tenantId}/guage/$metric/data",
    ...
    badGet(path: "${tenantId}/guage/$metric/data", query: [buckets: 1, bucketDuration: "1d"]) { exception ->
  ...
  void largeDataSetTest() {
    ...
    for (step in 1..NUMBER_OF_BUCKETS) {
      ...
      def response = hawkularMetrics.post(path: "$tenantId/guage/$metric/data", body: data)
      assertEquals(200, response.status)
    }

    def response = hawkularMetrics.get(path: "${tenantId}/guage/$metric/data",
